...
names(countMatrix) <- c("index", sub("ReadsPerGene.out.tab","",basename(count.files[1])))
...
if (length((all_samples)) != length(count.files)){
  omitted <- count.files[!(basename(gsub(pattern = "\\ReadsPerGene.out.tab", "", count.files)) %in% all_samples)]
  print("WARNING: Some samples have been omitted from samples.csv. Resuming without these samples:")
  for (i in seq(omitted)){
    print(basename(gsub(pattern = "\\.ReadsPerGene.out.tab", "", omitted[i])))
    
  }
  count.files <- count.files[basename(gsub(pattern = "\\ReadsPerGene.out.tab$", "", count.files)) %in% all_samples]
}
